...
- **Требуемые IOPS**: 5,886.3  
  `Расчёт: (0.23 + 8.1 + 70 + 4827) × 1.2`
- **Требуемый объём**: 16.4 ТБ/год  
  `Расчёт: 2250 × 0.23 × 86400 × 365 = 16.4 ТБ`
- **Конфигурация дисков**: SSD 2 ТБ (500 МБ/с, 1,000 IOPS)
  
  | Критерий      | Формула                     | Результат | Дисков |
  |---------------|-----------------------------|-----------|--------|
  | По ёмкости    | 16.4 ТБ ÷ 2 ТБ              | 8.2       | 9      |
  | По пропускной | (11,000 Мбит/с ÷ 8) ÷ 500 МБ/с | 2.75      | 3      |
  | По IOPS       | 5,886.3 ÷ 1,000             | 5.89      | 6      |

**Итог**: 6 дисков SSD 2Tb для хранения данных за последний месяц, 16 HDD 1TB для охлаждения данных.

#### Основная база данных (SQL)

- **Требуемые IOPS**: 5,886.3  
  `Расчёт: (0.23 + 8.1 + 70 + 4827) × 1.2`
- **Требуемый объём**: 0.3 ТБ/год  
  `Расчёт: (4 КБ × 86,400 сек/день) × 365 дней`
- **Конфигурация дисков**: SSD 1 ТБ (500 МБ/с, 1,000 IOPS)
  
  | Критерий      | Формула               | Результат | Дисков |
  |---------------|-----------------------|-----------|--------|
  | По ёмкости    | 0.3 ТБ ÷ 1 ТБ         | 0.3       | 1      |
  | По пропускной | 23.5 МБ/с ÷ 500 МБ/с | 0.05      | 1      |
  | По IOPS       | 5,886.3 ÷ 1,000       | 5.89      | 6      |

**Итог**: 6 дисков SSD 1 ТБ

#### Elasticsearch (хранилище постов)

- **Требуемые IOPS**: 5,886.3  
  `Расчёт: (0.23 + 8.1 + 70 + 4827) × 1.2`
- **Требуемый объём**: 0.035 ТБ/год  
  `Расчёт: 5 КБ × 0.23 пост/сек × 86,400 сек/день × 365 дней`
- **Конфигурация дисков**: NVMe SSD 2 ТБ (3,000 МБ/с, 10,000 IOPS)
  
  | Критерий      | Формула               | Результат | Дисков |
  |---------------|-----------------------|-----------|--------|
  | По ёмкости    | 0.035 ТБ ÷ 2 ТБ       | 0.02      | 1      |
  | По пропускной | 30 МБ/с ÷ 3,000 МБ/с | 0.01      | 1      |
  | По IOPS       | 5,886.3 ÷ 10,000      | 0.59      | 1      |

**Итог**: 1 диск NVMe SSD 2 ТБ


### Расчет количества хостов
Фактор репликации 3

| Компонент  | Кол-во хостов | Дисковая конфигурация             | Общий объём данных | Примечания                     |
|------------|---------------|-----------------------------------|--------------------|--------------------------------|
| **Elastic**| 3             | 1 × NVMe SSD 2 ТБ на хост         | 6 ТБ               | Фактор репликации 3            |
| **SQL**    | 9             | 1 × SSD 1 ТБ на хост              | 18 ТБ              | 6 шардов × 3 реплики           |
| **Итого**  | **12 хостов** |                                   |                    |                                |

**Пояснения:**
1. **Elasticsearch (3 хоста):**
   - Каждый хост: 2 ТБ NVMe SSD
   - Данные распределены: 1 первичный шард + 2 реплики других шардов на каждом узле
   - Минимум 3 ноды для обеспечения отказоустойчивости

2. **SQL (9 хостов):**
   - 3 ноды на каждые 2 шарда
   - Каждый хост содержит 1 ТБ данных (1/2 от общего объёма группы)
